...
/**
 * Settings list page rendered in the narrow activity-bar sidebar. The wide
 * settings panel no longer renders its own navigation; clicking an item here
 * opens the panel on that tab (see SettingsNavProvider in the extension).
 *
 * Labels go through the webview `LanguageProvider`, so switching the language
 * in the settings panel re-localizes this list reactively (the extension
 * pushes the effective locale via `navLanguage`).
 */
const SettingsNav: Component = () => {
  const vscode = useVSCode()
  const language = useLanguage()
  const [active, setActive] = createSignal<string>("providers")

  const open = (tab: string) => {
    setActive(tab)
    vscode.postMessage({ type: "openSettingsPanel", tab })
  }

  return (
    <div class="settings-nav">
      <style>{`
        .settings-nav { flex: 1; display: flex; flex-direction: column; gap: 8px; padding: 8px; min-height: 0; overflow-y: auto; }
        .settings-nav-item { flex: 1 1 0; min-height: 40px; max-height: 56px; display: flex; align-items: center; gap: 10px; padding: 0 12px; border-radius: 6px; cursor: pointer; font-size: 13px; line-height: normal; white-space: nowrap; overflow: hidden; color: var(--vscode-foreground); }
        .settings-nav-item:hover, .settings-nav-item:focus-visible { background: var(--vscode-list-hoverBackground); outline: none; }
        .settings-nav-item.is-active { background: var(--vscode-list-activeSelectionBackground); color: var(--vscode-list-activeSelectionForeground); }
        .settings-nav-item svg { width: 18px; height: 18px; flex-shrink: 0; color: currentColor; }
        .settings-nav-item span { overflow: hidden; text-overflow: ellipsis; }
      `}</style>
      {NAV_ITEMS.map((item) => (
        <div
          class="settings-nav-item"
          classList={{ "is-active": active() === item.tab }}
          data-tab={item.tab}
          role="button"
          tabIndex={0}
        ...
        >
          {item.stroke ? (
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-linecap="square" aria-hidden="true">
              <path d={item.icon} />
            </svg>
          ) : (
          ...
          <span>{language.t(`settings.${item.tab}.title`)}</span>
        </div>
      ))}
    </div>
  )
